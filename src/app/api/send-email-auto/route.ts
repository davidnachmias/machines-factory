...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    // בדיקת הגדרות הדוא"ל
    if (!process.env.GMAIL_USER || !process.env.GMAIL_PASS || !process.env.GMAIL_SUPPORT) {
    ...
    const errorMessage = error instanceof Error ? error.message : "שגיאה לא ידועה בשליחת המייל";
...
export async function GET(): Promise<NextResponse> {
  ...
  try {
    ...
    return NextResponse.json({ message: "דוח תקופתי נשלח בהצלחה!" }, { status: 200 });
  } catch (error: unknown) {
